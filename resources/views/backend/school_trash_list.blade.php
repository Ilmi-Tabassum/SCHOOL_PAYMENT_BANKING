   ...
   <div id="page-content" style="margin-top: 0px;margin-left: 20px">
     ...
     <section class="content-header" style="margin-right: 1%;height: 50px">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3 style="font-size: 25px;font-weight: bolder;margin-left: -15px">School Trash List</h3>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{url('/')}}">Home</a></li>
              <li class="breadcrumb-item active">School Trash List</li>
            </ol>
          </div>
        </div>
      </div>
        ...
        foreach ($schools as $key => $value) {
          ...
          if($value->status == 5){
            ...
            if($value->status == 4){
              ...
              if($value->status == 3){
                $table_option .= "<span class='badge badge-success'>Deleted</span>";
              }else{
                if($value->status == 2){
          ...
          if($value->status == 3)
              {
                  $table_option .= "<td><a href='#editSchool' class='tooltip-button editSchoolInfo' id='" .$value->id. "' data-original-title='Edit' style='padding-right: 10px' data-toggle='modal' data-target='#modal-default'>
          <i class='nav-icon fas fa-edit text-warning'></i></a>
          <a href='". route('school_info.restore', $value->id) ."' class='tooltip-button confirm_restore_dialog' data-original-title='Restore'><i class='nav-icon fas fa-window-restore text-success'></i></a></td>";
